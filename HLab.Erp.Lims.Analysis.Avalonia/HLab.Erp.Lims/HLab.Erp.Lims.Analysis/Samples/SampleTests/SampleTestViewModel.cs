...
public class SampleTestViewModel : EntityViewModel<SampleTest>, IMvvmContextProvider
{
   ...
   ) : base(i)
   {
      ...
      this.WhenAnyValue(
          ...
          {
             var (locker, sample) = v;
             if(locker is null) return false;
      ...
      this.WhenAnyValue(e => e.AuditDetail, e => e.AuditTrail)
          .Subscribe(e =>
          {
             var (auditDetail, auditTrail) = e;
             OnAuditDetailChanged(auditDetail, auditTrail);
          });
      ...
      _resultMode = this.WhenAnyValue(
         e => e.FormHelper.Form,
         e => e.Injected.Acl,
         e => e.Locker.IsActive,
         e => e.Workflow.CurrentStage,
         GetResultMode)
         .ToProperty(this, x => x.ResultMode);

      _formHelperIsActive = this.WhenAnyValue(
          e => e.EditMode,
          e => e.ResultMode,
   ...
   public async Task LoadResultAsync(IFormTarget? target = null)
   {
      ...
      if(FormHelper.Form is null) return;

      FormHelper.Form.Mode = Workflow.CurrentStage == SampleTestWorkflow.Specifications ? FormMode.Specification : FormMode.ReadOnly;
   ...
   readonly ObservableAsPropertyHelper<SampleTestAuditTrailViewModel> _auditTrail;
   ...
   static void OnAuditDetailChanged(bool auditDetail, SampleTestAuditTrailViewModel? auditTrail)
   {
   ...
   {
      if(form is null) return false; // TODO : how to not have this test ?
