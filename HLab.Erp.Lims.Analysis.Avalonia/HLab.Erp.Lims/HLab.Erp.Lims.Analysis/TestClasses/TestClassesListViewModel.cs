...
public class TestClassesListViewModel(IAclService acl, EntityListViewModel<TestClass>.Injector i)
   : EntityListViewModel<TestClass>(i, c => c
      .Column("Name")
      ...
      .OrderBy(s => s.Category?.Name)), IMvvmContextProvider
{
    ...
    protected override bool AddCanExecute(Action<string> errorAction) =>
        acl.IsGranted(AnalysisRights.AnalysisTestClassCreate, errorAction);

    protected override bool DeleteCanExecute(TestClass arg, Action<string> errorAction) =>
        arg!=null && acl.IsGranted(AnalysisRights.AnalysisTestClassCreate, errorAction);
