...
using H = DependencyHelper<TestForm>;

public abstract class TestForm : UserControl, IForm
{
    ...
    public void TryProcess(object sender, EventArgs args)
    {
        ...
        {
            SetErrorMessage(new TestClassesExceptionView { DataContext = e });
        }
        finally
        {
    ...
    public static readonly DependencyProperty TargetProperty = H.Property<IFormTarget>().Register();

    public IFormTarget Target
    {
        get => (IFormTarget)GetValue(TargetProperty);
        set => SetValue(TargetProperty, value);
    }

    public long CreationDuration { get; set; }

    public static readonly DependencyProperty ModeProperty = H.Property<FormMode>()
        .OnChange((e, a) => e.SetFormMode(a.NewValue))
        .Register();

    public FormMode Mode
    {
        get => (FormMode)GetValue(ModeProperty);
    ...
    public void SetFormMode(FormMode mode)
    {
        ...
        foreach (var element in NamedElements.OfType<FrameworkElement>())
        {
    ...
    {
        foreach (var c in NamedElements.OfType<FrameworkElement>())
        {
            var name = c.Name;
