...
namespace HLab.Erp.Lims.Analysis.Data.Entities
{
   public class ProductComponent : Entity, IListableModel
   {
      ...
      [Ignore] public string Caption => $"{Inn?.Caption} {Quantity,0} {Unit?.Symbol}";
      ...
      public int? InnId { get => _inn.Id; set => _inn.SetId(value); }
      [Ignore] public Inn Inn { get => _inn.Value; set => InnId = value?.Id; }
      readonly ForeignPropertyHelper<ProductComponent, Inn> _inn;

      public double Quantity { get; set => this.SetAndRaise(ref field, value); } = 0.0;

      public int? UnitId { get => _unit.Id; set => _unit.SetId(value); }
      [Ignore] public Unit? Unit { get => _unit.Value; set => UnitId = value?.Id; }
